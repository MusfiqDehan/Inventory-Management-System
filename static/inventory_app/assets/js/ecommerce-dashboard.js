(function (factory) {
    ...
    echartTabs && echartTabs.forEach((e) => {
        e.addEventListener("shown.bs.tab", (e) => {
            const t = e.target;
            const { hash: o } = t;
            const r = o || t.dataset.bsTarget;
            const a = document.getElementById(r.substring(1))?.querySelector("[data-echart-tab]");

            if (a) {
                window.echarts.init(a).resize();
            }
        });
    });
